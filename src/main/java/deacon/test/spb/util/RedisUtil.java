...
public class RedisUtil {
    ...
    /**
     * 验证redis中存储的数据同步状态，状态为null或1时，表示可以进行同步；否则不需要进行同步。
     * 主要用于防止首次数据同步时间较长，超过同步周期，会重复同步的情况
     * 
     * @return
     */
    public boolean checkSyncState() {
        Integer sync = (Integer)this.get("syncState");
    ...
    public <T> T runLua(String fileClasspath, Class<T> returnType, List<String> keys, Object... values) {
        ...
        return (T)redisTemplate.execute((RedisScript)redisScript, redisTemplate.getKeySerializer(),
            (RedisSerializer<Object>)redisTemplate.getValueSerializer(), keys, values);
    }
	 ...
	 * @return
	 */
	public Object hgetCacheVagueItem(String key, String item) {
		try {
		    Cursor<Map.Entry<Object,Object>> cursor = redisTemplate.opsForHash().scan(key,
		    ScanOptions.scanOptions().match("*" + item + "*").count(1).build());
